...
;(function( $, document, window, undefined ) {
	...
	var AvalancheSlider = function( element, options ){
		...
		settings.muteBkgVideo = slider.is( '[data-mute-bkg-video="false"]' ) ? false : settings.muteBkgVideo;
		if( mobile ){
	...
	*/
	...
	var updateParallaxElement = function( slider, settings ){
		...
		if( !mobile && tSupport && isElementVisible( slider.parent() ) ){
			slider.css({ position: 'fixed', transform: 'translate3d( 0, ' + -distance + 'px' + ', 0) scale3d( ' + scale + ',' + scale + ', 1 )', opacity: opacity.toFixed(2) });
		}else{
